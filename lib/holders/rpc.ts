...
import { WsTransport, httpTwin, isWsUrl } from "./ws";
...
export class RpcClient {
  ...
  private wsTransports = new Map<string, WsTransport>();

  constructor(urls: string[], private readonly timeoutMs = 15_000) {
    // A wss:// endpoint usually has an https:// twin on the same host, and
    // HTTP suits a serverless runtime better — nothing to keep open between
    // invocations. Try that first, but keep the WebSocket URL behind it so a
    // WebSocket-only provider still works.
    const expanded: string[] = [];
    for (const u of urls) {
      const twin = httpTwin(u);
      if (twin) expanded.push(twin);
      expanded.push(u);
    }
    this.urls = [...new Set(expanded)];
  }

  private wsFor(url: string): WsTransport {
    let t = this.wsTransports.get(url);
    if (!t) {
      t = new WsTransport(url, this.timeoutMs);
      this.wsTransports.set(url, t);
    }
    return t;
  }
  ...
  async batch<T>(requests: RpcRequest[]): Promise<(T | Error)[]> {
    ...
    for (let attempt = 0; attempt < this.urls.length; attempt++) {
      ...
      try {
        ...
        let json: unknown;
        if (isWsUrl(url)) {
          json = await this.wsFor(url).send(body);
        } else {
